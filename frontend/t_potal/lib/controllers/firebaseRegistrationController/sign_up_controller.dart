...
import '../techerDataFromNode/teacherInfo/teacher_json.dart';
  ...
  Future<void> additionalUserInfo() async {
    try {
      final teacherData = TeacherJson(
          ...
          verify: false,
          earn: 0);
      await teacherCrudController.createTeacher(teacherData);
    } catch (e) {
      Get.snackbar(
        'Error',
...
//   // Function to check if the username already exists in Firestore
  Future<void> checkMcid() async {
    try {
      await teacherCrudController.fetchItems();
      // ignore: invalid_use_of_protected_member
      final list = teacherCrudController.teachers.value;
      if (list.isNotEmpty) {
        for (final teacher in list) {
          if (teacher.mcid == messageCallID.text) {
